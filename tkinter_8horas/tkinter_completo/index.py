...
class funcs():  # criada para limpar as entrys
    ...
    def conecta_bd(self):  # criando a função chamando o banco de dados
        ...
        # o CONN  é uma função de conexão do banco
    ...
    def monta_tabelas(self):  # função de montar tabelas
        ...
        """)
        self.conn.commit()  # Validando o banco de dados no sistema
        print('Banco de dados criado')  # mensagem de criação do banco de dados
        self.desconecta_bd()  # desconectando o banco de dados

    def variaveis(self):  # sta função e as variaveis foram alteradas para economizar linhas de codigo
    ...
    def add_cliente(self):  # Função de insert ou  add dados atraves de variaveis nas entrys
        ...
        self.select_lista()
        # vai limpar as entrys para novos dados AQUI O MEU ESTA DIFERENTE.
        self.limpa_tela()

    def select_lista(self):
        # listaCli esta dentro da lista_frame2
        # esta função limp a o que ja tem na lista
    ...
    def OnDoubleClick(self, event):  # criando a função de duplo click e preciso pro o event
        self.limpa_tela()  # estou utilizando para se houver alguam coisa nas entrys será limpo
        self.listaCli.selection()  # esse comando vai selecionar as informações dessa lista

        for n in self.listaCli.selection():  # esse for serve para extrair os dados das colunas abaixo
            col1, col2, col3, col4 = self.listaCli.item(n, 'values')
            self.codigo_entry.insert(END, col1)
            self.nome.insert(END, col2)
            self.fone.insert(END, col3)
            self.cidade.insert(END, col4)

    def deleta_cliente(self):  # tenho que por o command la no frame1
        self.variaveis()  # preciso por essa função aqui dentro senao não funciona
        self.conecta_bd()  # como sempre conectando para poder executar a funçao
        self.cursor.execute("""DELETE FROM clientes WHERE cod = ?""",
                            (self.codigo))  # precisa ser igual a isso
        self.conn.commit()  # finaliza o processo para serguir para o próximo passo
        self.desconecta_bd()  # como sempre desconectando o banco de dadose finalizando o processo
        self.limpa_tela()  # após a execução limpa a tela
        self.select_lista()  # chamando essa função ela ataliza as informações das treeview
...
# TERCEIRO PASSO -  criando as configuração da tela onde vou alocar meus widgets*********************************************
    ...
    def widgets_frame1(self):
        ...
        self.btn_apagar = Button(self.frame_1, text='Apagar', bd=2,
                                 bg='#187db2', fg='white', font=('Verdana', 8, 'bold'), command=self.deleta_cliente)
...
# CRIANDO FUNÇÕES ENTRO DO FRAME 2 QUE É A PARTE DE BAIXO DA TELA DOS NOSS SISTEMA
    ...
    # Eabaixo eu identifico  o nome de cada coluna
...
# Que pertence ao Frame2 e sua orientação é vertical
        ...
        # essa função criainteração com a lista atraves do doubleclic
